...
> このスキルは同じ基準で構成された Zenn リポジトリ間で共通。リポジトリ固有の慣習（コミット件名の実例、公開状態、画像ディレクトリの方針）は `.claude/zenn-profile.md` にある。
...
- 件名は**既存履歴の書式に揃える**。プレフィックスは付けない。実例はプロファイルの「コミット履歴の慣習」を見る。迷ったら `git log --oneline -20` で直近の書き方を確認する
...
> **`main` への push は Zenn への本番デプロイ。** `published: true` のコンテンツに触れた変更は、マージした時点で公開ページに反映される。何が公開済みかはプロファイルの「公開状態」で確認し、マージ前に `npx zenn preview` で最終確認する。
...
`git status` の `??` は機械的に `git add -A` せず、中身を見て判断する。
...
**本のチャプターに画像を貼った場合は要注意。** Paste Image は `images/<チャプターのファイル名>/` に保存するが、本の画像を本の slug 単位に集約している場合は移動と参照パスの書き換えが必要。コミット前に済ませる（プロファイルの「画像ディレクトリの方針」と `CLAUDE.md` の「画像」節を参照）。
