...
エラーハンドリング繋がりということで「トランザクション処理とtry...catch構文」をご紹介しておきます。

まずは、トランザクション処理とtry...catch構文とは何かを確認してみましょう。
下記の記事では「「transactionメソッド」を使う方法が紹介されています。
シンプルで分かりやすいので一読しておきましょう。
https://www.happylifecreators.com/blog/20220513/

また、下記の記事では「beginTransactionメソッド」を使う方法も紹介されています。
今ではアンチパターンとされてますが知識としては知っておくと良いと思います。
今でも良く見かける実装方法だからです。
https://shinyasunamachi.com/blog/Laravelでトランザクション処理のエラーハンドリングを行う方法

それではToDoアプリでも「トランザクション処理」を実装してみましょう。
...
次に、try...catch構文について考えてみます。
今回は、`laravel.log` にログを記録することにしたいと考えてます。
但し、Laravelは親切なのでtry...catch構文を記述しなくれも例外をスローして記録してくれます。
そうなると特殊な要件でもないのであればtry...catch構文は必要ないということになります。
しかし、ここでは練習だと思ってtry...catch構文を記述していくことにします。
記録が目的ですので自分で作ったコントローラーのメソッドすべてが対象範囲になります。
...
これで「try...catch構文とトランザクション処理」のエラーハンドリングは完了しました。
